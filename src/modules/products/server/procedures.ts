...
export const productsRouter = createTRPCRouter({
  getOne: baseProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const reviewRating =
        reviews.docs.length > 0 ? reviews.docs.reduce((acc, review) => acc + review.rating, 0) / reviews.totalDocs : 0;
  ...
  getMany: baseProcedure
    .input(
      z.object({
        ...
        limit: z.number().min(1).default(DEFAULT_LIMIT),
    ...
    .query(async ({ ctx, input }) => {
      const where: Where = {
        isArchived: {
          not_equals: true, // instead of equals: false, to prevent errors where defaultValue is set to null
        },
      };
      ...
      if (input.tenantSlug) {
        ...
        // if we're loading products for public storefront (no tenantSlug)
      ...
      const data = await ctx.payload.find({
        collection: "products",
        depth: 2, // populates "image", "category", tenant & tenant.image
        where,
        sort,
        page: input.cursor,
        limit: input.limit,
